...
void game_start(GameConfig *config) {
    ...
    init_maze(&maze, config->width, config->height);
    generate_maze_wilson(&maze);
    init_hint(&hint, &hint_timer, config->hint_duration, getpid(), SIGUSR2);
    ...
    if (config->time_limit > 0) {
        ...
        init_timer(&maze_timer, config->time_limit, getpid(), SIGUSR1);
        start_timer(&maze_timer);
    }

    print_game(&maze, get_time_left(maze_timer));

    while (game_should_continue(player, maze, maze_timer, config->time_limit)) {
    ...
    print_game(&maze, get_time_left(maze_timer));
    print_game_result(is_exit_reached(player, maze), config->time_limit, get_time_left(maze_timer));
